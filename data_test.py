...
print(overall_scores_df.sort_values(by='Total_Points', ascending=False))
...
app.layout = html.Div(
    style={
        "display": "grid",
        "grid-template-columns": "20vw 30vw 30vw",  # Добавлена третья колонка
        "gap": "20px",
        "height": "auto",
        "width": "fit-content",
        "padding": "20px",
            ...
            children=[
                ...
                html.Div(
                 ...
                 html.Div(
                    ...
                    style={'height': "auto", "width": "30vw", 'box-sizing': 'border-box'} # Изменена ширина
                ),
            ]
        ),
        html.Div( # Новая колонка для gym_points
            style={
                'grid-column': '3',
                'display': 'flex',
                'flex-direction': 'column',
...
def display_clicked_districts(clickData):
    if clickData is not None:
        ...
        # Assuming 'Number' is the correct column name for count in 'air' DataFrame
        # If 'Number of Parks' is the correct column name for count, use that instead.
        # Based on air = sorted_airs.groupby("Number of Parks")["District"].apply(list).reset_index()
        # The column will be 'Number of Parks'
        return html.Div([
